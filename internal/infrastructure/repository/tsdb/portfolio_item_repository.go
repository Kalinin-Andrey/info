...
const (
	portfolio_item_sql_MGet                      = "SELECT portfolio_source_id, currency_id, amount, current_price, crypto_holdings, holdings_percent, buy_avg_price, pl_percent_value, pl_value, updated_at FROM cmc.portfolio_item WHERE portfolio_source_id = $1;"
	portfolio_item_sql_MCreate                   = "INSERT INTO cmc.portfolio_item(portfolio_source_id, currency_id, amount, current_price, crypto_holdings, holdings_percent, buy_avg_price, pl_percent_value, pl_value, updated_at) VALUES "
	portfolio_item_sql_Create_OnConflictDoUpdate = " ON CONFLICT (portfolio_source_id, currency_id) DO UPDATE SET amount = EXCLUDED.amount, current_price = EXCLUDED.current_price, crypto_holdings = EXCLUDED.crypto_holdings, holdings_percent = EXCLUDED.holdings_percent, buy_avg_price = EXCLUDED.buy_avg_price, pl_percent_value = EXCLUDED.pl_percent_value, pl_value = EXCLUDED.pl_value, updated_at = EXCLUDED.updated_at;"
)
...
func (r *PortfolioItemRepository) MGetByPortfolioSourceId(ctx context.Context, portfolioSourceId uint) (*portfolio_item.PortfolioItemMap, error) {
	...
	for rows.Next() {
		if err = rows.Scan(&entity.PortfolioSourceID, &entity.CurrencyID, &entity.Amount, &entity.CurrentPrice, &entity.CryptoHoldings, &entity.HoldingsPercent, &entity.BuyAvgPrice, &entity.PlPercentValue, &entity.PlValue, &entity.UpdatedAt); err != nil {
			r.metrics.SqlMetrics.Inc(metricName, metricsFail)
			r.metrics.SqlMetrics.WriteTiming(start, metricName, metricsFail)
			return nil, fmt.Errorf("[%w] %s query error; query: %s; error: %w", apperror.ErrInternal, metricName, portfolio_item_sql_MGet, err)
		}
...
func (r PortfolioItemRepository) MUpsert(ctx context.Context, entities *portfolio_item.PortfolioItemList) error {
	...
	for i, entity := range *entities {
		...
		b.WriteString("($" + strconv.Itoa(i*fields_nb+1) + ", $" + strconv.Itoa(i*fields_nb+2) + ", $" + strconv.Itoa(i*fields_nb+3) + ", $" + strconv.Itoa(i*fields_nb+4) + ", $" + strconv.Itoa(i*fields_nb+5) + ", $" + strconv.Itoa(i*fields_nb+6) + ", $" + strconv.Itoa(i*fields_nb+7) + ", $" + strconv.Itoa(i*fields_nb+8) + ", $" + strconv.Itoa(i*fields_nb+9) + ", $" + strconv.Itoa(i*fields_nb+10) + ")")
		params = append(params, entity.PortfolioSourceID, entity.CurrencyID, entity.Amount, entity.CurrentPrice, entity.CryptoHoldings, entity.HoldingsPercent, entity.BuyAvgPrice, entity.PlPercentValue, entity.PlValue, entity.UpdatedAt)
	}
